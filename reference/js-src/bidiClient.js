...
class BidiCursorClient extends EventEmitter {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      this.session.on('error', (err) => {
        logger.error('HTTP/2 session error:', err.message);
        reject(err);
      });
      ...
      this.session.on('goaway', () => {
        logger.debug('Server sent GOAWAY');
      });
   ...
   * Encode tool-specific result data for sending back to Cursor
   */
  encodeToolSpecificResult(tool, result) {
    ...
    switch (tool) {
      ...
      case ClientSideToolV2.RUN_TERMINAL_COMMAND_V2: {
        ...
        let msg = Buffer.alloc(0);
        // Result format: { output, exit_code } or { stdout, stderr, exit_code }
  ...
  async runAgent(authToken, prompt, model = 'claude-4-sonnet', options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (verbose) {
        logger.info(`Agent mode (bidi) with model: ${model}`);
        logger.debug(`Sent initial request (${requestBody.length} bytes)`);
      }
      ...
      stream.on('data', async (chunk) => {
        ...
        if (verbose) logger.debug(`[Received ${chunk.length} bytes, buffer: ${buffer.length}]`);
        ...
        for (const { compressed, payload } of frames) {
          ...
          for (const tc of toolCalls) {
            if (!toolCallsSeen.has(tc.toolCallId)) {
              ...
              if (toolCallsExecuted < maxToolCalls) {
                ...
                logger.debug(`[BidiClient] Tool execution skipped (local execution removed): ${tc.name}`);
                if (verbose) {
                  logger.debug(`[BidiClient] Tool call not executed: local tool executor has been removed`);
                }
              }
            }
          }
          ...
          if (toolCalls.length === 0) {
            ...
            if (response.thinking && verbose) {
              logger.debug(`<thinking>${response.thinking}</thinking>`);
            }
            ...
            if (verbose && !response.text && data.length > 20) {
              try {
                ...
                logger.debug(`[No text, fields: ${fieldNums}, size: ${data.length}]`);
              } catch (e) {}
            }
          }
        }
      ...
      stream.on('end', () => {
        ...
        if (verbose) {
          ...
          if (toolCallsExecuted > 0) {
            logger.debug(`Executed ${toolCallsExecuted} tool call(s)`);
          }
        }
        this.close();
        resolve(fullResponse);
      ...
      stream.on('response', (headers) => {
        if (verbose) {
          logger.debug('[Response headers received]');
