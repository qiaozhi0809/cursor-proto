...
const { saveToken, TOKEN_FILE } = require('../utils/tokenManager');
  ...
  async function main(){
    ...
    for (let i = 0; i < retryAttempts; i++) {
      ...
      if (data) {
        ...
        console.log("[Log] Your Cursor token:");
        console.log(token);
        
        // 自动保存 token 到本地
        if (saveToken(token)) {
          console.log(`[Log] Token saved to: ${TOKEN_FILE}`);
          console.log("[Log] The server will automatically use this token for requests.");
        }
        break;
      }
